...
import com.example.arabicsignlanguage.databinding.ActivityLetterCameraBinding
import java.io.ByteArrayOutputStream
import java.nio.ByteBuffer
import java.util.concurrent.ExecutorService
import java.util.concurrent.Executors

class LetterCameraActivity : AppCompatActivity(), Detector.DetectorListener {
    
    private lateinit var binding: ActivityLetterCameraBinding
    private var imageCapture: ImageCapture? = null
    private lateinit var cameraExecutor: ExecutorService
    private lateinit var detector: Detector
    private var isDetectorInitialized = false
    private var isFinishing = false
    private val TAG = "LetterCameraActivity"
    
    // For approved functionality
    private var currentDetectedLetter: String = ""
    private var currentConfidence: Float = 0f
    
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        Log.d(TAG, "=== LETTER CAMERA ACTIVITY CREATED ===")
        
        binding = ActivityLetterCameraBinding.inflate(layoutInflater)
        ...
        onBackPressedDispatcher.addCallback(this, object : OnBackPressedCallback(true) {
            override fun handleOnBackPressed() {
                Log.d(TAG, "Hardware back button pressed in LetterCameraActivity")
    ...
    private fun setupUI() {
        ...
        binding.btnBack.setOnClickListener {
            Log.d(TAG, "Back button clicked in LetterCameraActivity")
            navigateBackToTranslationSelection()
        }
        ...
        // Title
        binding.tvTitle.text = "ترجمة الحروف"
        binding.tvSubtitle.text = "ضع يدك أمام الكاميرا لترجمة الحروف"
        ...
        // Disable keyboard for statement EditText
        binding.etStatement.showSoftInputOnFocus = false
        binding.etStatement.isFocusable = true
        binding.etStatement.isFocusableInTouchMode = true
    }
    
    private fun setupDetector() {
        try {
            Log.d(TAG, "Setting up TensorFlow detector for LETTER mode")
            // Use TensorFlow Object Detection API detector for letters
            detector = Detector(baseContext, Constants.LETTER_MODEL_PATH, Constants.LETTER_LABELS_PATH, this)
            detector.setup()
            isDetectorInitialized = true
            Log.d(TAG, "TensorFlow detector setup completed for LETTER mode")
        } catch (e: Exception) {
    ...
    private fun startCamera() {
        ...
        cameraProviderFuture.addListener({
            try {
                ...
                val imageAnalyzer = ImageAnalysis.Builder()
                    ...
                    .also {
                        it.setAnalyzer(cameraExecutor, { imageProxy ->
                            processImageForLetters(imageProxy)
                        })
                    }
                ...
                Log.d(TAG, "Letter camera started successfully")
                
            } catch (exc: Exception) {
                Log.e(TAG, "Letter camera startup failed", exc)
                showCameraError()
            }
        }, ContextCompat.getMainExecutor(this))
    }
    
    private fun processImageForLetters(imageProxy: ImageProxy) {
        ...
        try {
            ...
            // Run detection
            if (::detector.isInitialized && !isFinishing) {
    ...
    // Detector.DetectorListener implementation
    override fun onEmptyDetect() {
        if (!isFinishing && !isDestroyed) {
            runOnUiThread {
                if (!isFinishing && !isDestroyed && ::binding.isInitialized) {
                    try {
                        binding.tvDetectedLetter.text = "الحرف المكتشف: -"
    ...
    private fun showClearOptions() {
        val currentText = binding.etStatement.text.toString()
        ...
        // Option to clear last letter
        if (currentText.isNotEmpty()) {
            options.add("مسح آخر حرف")
            actions.add { clearLastLetter() }
        }
    ...
    private fun clearLastLetter() {
        ...
        if (currentText.isNotEmpty()) {
            ...
            binding.etStatement.setText(newText)
            Log.d(TAG, "Cleared last letter. New text: '$newText'")
        }
    }
    ...
    private fun addLetterToStatement() {
        if (currentDetectedLetter.isNotEmpty()) {
            val currentText = binding.etStatement.text.toString()
            val newText = currentText + currentDetectedLetter
            binding.etStatement.setText(newText)
            
            Log.d(TAG, "Added letter '$currentDetectedLetter' to statement: '$newText'")
    ...
    private fun navigateBackToTranslationSelection() {
        Log.d(TAG, "=== NAVIGATING BACK TO TRANSLATION SELECTION FROM LETTER CAMERA ===")
        ...
        // إنهاء الـ LetterCameraActivity
        finish()
        
        Log.d(TAG, "LetterCameraActivity finished")
    }
    
    override fun onDestroy() {
        super.onDestroy()
        Log.d(TAG, "LetterCameraActivity destroyed")
        
        isFinishing = true
        
        try {
            if (isDetectorInitialized && ::detector.isInitialized) {
                Log.d(TAG, "Clearing TensorFlow detector in onDestroy")
                detector.clear()
            }
        } catch (e: Exception) {
